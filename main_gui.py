...
class BrailleApp:
    def __init__(self, root):
        ...
        # Initialize translator and state variables
        self.logic = BrailleTranslator()  # Core Braille processing engine
        self.decoded_text = ""  # Stores the decoded Braille text
        self.gt_path = ""  # Path to ground truth text file
        self.reference_image = None  # Holds the reference image for display
        self.reference_photo = None  # PhotoImage object for Tkinter
        self.reference_loaded = False  # Flag to track if reference is loaded
        
        # Configure grid weights for responsive two-panel layout
        ...
        self.create_left_panel()    # Controls and output
        self.create_right_panel()    # Reference chart display
        
        # Automatically load reference image after UI is ready
    ...
    def load_default_reference(self):
        ...
        # Priority list of filenames to try (most common first)
        possible_files = [
            "decode_pattern.jpg",
            "decode_pattern.jpeg", 
            "decode_pattern.png",
        ...
        # Try each filename until one loads successfully
        ...
        # No image could be loaded - show informational message
    ...
    def show_no_reference_message(self):
        ...
        try:
            ...
            # Informational header (not an error)
            tk.Label(
                message_frame,
            ...
            # Reassure user that decoder still works
            tk.Label(
                message_frame,
            ...
            # Show current directory for troubleshooting
            tk.Label(
                message_frame,
            ...
            # Instructions for adding a chart later
            tk.Label(
                message_frame,
    ...
    def create_left_panel(self):
        ...
        # Button 1: Select Braille image to decode
        tk.Button(
            control_frame, 
            text="📷 Select Braille Image",
        ...
        # Button 2: Load ground truth for accuracy comparison
        tk.Button(
            control_frame, 
        ...
        # Button 3: Calculate accuracy metrics
        tk.Button(
            control_frame, 
        ...
        # Accuracy metrics display section
        ...
        # Detailed accuracy report section
        ...
        # Configure panel weights for proper resizing
    ...
    def _create_metric_card(self, parent, label, col):
        ...
        card.configure(width=120, height=100)
    ...
    def create_right_panel(self):
        ...
        tk.Label(
            header_frame, 
            text="Braille Reference Chart", 
            font=("Helvetica", 16, "bold"), 
            bg="#34495e", 
            fg="white"
        ).pack(expand=True)
        
        # Container with centering capability
        ...
        # Frame inside canvas to hold the image (centered)
        self.image_frame = tk.Frame(self.canvas, bg="#f8f9fa")
        self.canvas_window = self.canvas.create_window((0, 0), window=self.image_frame, anchor="center")
    ...
    def display_reference_image(self, image_path):
        ...
        try:
            ...
            # Handle different image modes (especially PNG with transparency)
            ...
            # Calculate scaled dimensions to fit panel
            max_width = 600
            max_height = 700
            ...
            # Create container for image and metadata
            ...
            info_text = f"File: {os.path.basename(image_path)} • {orig_width} x {orig_height}"
            tk.Label(
                content_frame,
                text=info_text,
                font=("Helvetica", 9),
            ...
            # Update scroll region and center
            self.image_frame.update_idletasks()
            self.canvas.configure(scrollregion=self.canvas.bbox("all"))
            ...
            tk.Label(
                error_frame,
                text="⚠️ Could not display image",
                font=("Helvetica", 12, "bold"),
                bg="#f8f9fa",
                fg="#e74c3c"
            ).pack(pady=10)
    ...
    def _set_tile(self, tile, value, good):
        """Update a metric card with new value and color"""
        # Green for good performance (>=60%), red for poor
    ...
    def calc_accuracy(self):
        """Calculate and display accuracy metrics by comparing with ground truth"""
        # Validation checks
        if not self.decoded_text:
            messagebox.showwarning(
                "No Output", 
                "Please decode a Braille image first."
            )
            return
        ...
        if not self.gt_path:
            messagebox.showwarning(
                "No Ground Truth", 
                "Please load a ground truth .txt file first."
            )
            return
        ...
        try:
            ...
            # Calculate character accuracy (ignoring spaces)
            pred_chars = ''.join(c for c in pred if c.isalnum())
            truth_chars = ''.join(c for c in truth if c.isalnum())
            matched_chars = sum(p == t for p, t in zip(pred_chars, truth_chars))
            char_acc = round(matched_chars / max(len(truth_chars), 1) * 100, 1)
            
            # Calculate word accuracy
            truth_words = truth.split()
            pred_no_spaces = ''.join(pred.split())
            
            # Align predicted words with ground truth
            pred_words = []
            idx = 0
            for w in truth_words:
                lw = len(w)
                pred_words.append(pred_no_spaces[idx:idx+lw])
                idx += lw
            
            # Full word matches
            full_matches = sum(1 for pw, tw in zip(pred_words, truth_words) if pw == tw)
            full_acc = round(full_matches / max(len(truth_words), 1) * 100, 1)
            
            # Partial matches (60% character match threshold)
            partial_matches = 0
            ...
            # Update metric tiles with results
            ...
            lines = [
                ...
                f"    Matched chars     : {matched_chars} / {len(truth_chars)}",
                "",
                ...
                "",
                f"  Predicted length    : {len(pred_chars)} chars, {len(pred_words)} words",
                f"  Ground truth length : {len(truth_chars)} chars, {len(truth_words)} words",
                "=" * 50,
            ]
            self._write_acc('\n'.join(lines))
            
        except Exception as e:
            messagebox.showerror("Error", f"Accuracy calculation failed: {str(e)}")
